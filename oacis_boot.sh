...
usage() {
    ...
    echo "  -j JUPYTER_PORT (default: 8888) : port used for Jupyterlab"
    echo "  --publish-port : publish OACIS and jupyter port to external host"
    echo "  --no-ssh-agent : disable sharing ssh-agent of host OS"
    echo "  --image-tag OACIS_IMAGE_TAG (default: latest) : the tag name of OACIS image. Image 'oacis/oacis_jupyterlab:<TAG>' is used."
...
if [ -n "${OACIS_VERSION}" ]; then
  ...
  docker build . -t oacis/oacis_jupyterlab:${OACIS_IMAGE_TAG} --build-arg OACIS_VERSION=${OACIS_IMAGE_TAG}
  cd $SCRIPT_DIR
fi
